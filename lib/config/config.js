...
module.exports = function (fs, path, url, convict) {
  ...
  var conf = convict({
    ...
    ipBlocklist: {
      ...
      logOnly: {
        doc: 'Flag to only log hits on ip blocklist',
        format: Boolean,
        default: true,
        env: 'IP_BLOCKLIST_LOGONLY'
      },
      listPath: {
        doc: 'Relative location of the csv file containing list of ip addresses to block',
        format: String,
        default: '../test/mocks/firehol_level1.netset',
        env: 'IP_BLOCKLIST_LIST_PATH'
      }
    }
  })
