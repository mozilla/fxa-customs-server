...
module.exports = function createServer(config, log) {
  ...
  api.post(
    ...
    function (req, res, next) {
      ...
      if (!email || !ip) {
        var err = {code: 'MissingParameters', message: 'email, ip are all required'}
        log.error({ op: 'request.resetEntry', email: email, ip: ip, err: err })
        res.send(400, err)
        return next()
      }
      email = normalizedEmail(email)
      ...
      fetchRecords(email, ip)
        ...
        .then(
          function () {
            log.info({ op: 'request.resetEntry', email: email, ip: ip})
            res.send({})
          },
          function (err) {
            log.error({ op: 'request.resetEntry', email: email, ip: ip, err: err })
