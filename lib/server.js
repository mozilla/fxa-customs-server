      ...
      fetchRecords(email, ip)
        ...
        .then(
          function (result) {
            ...
            if (config.reputationService.enable && result.lockout) {
              ipClient.sendViolation(ip, 'fxa:request.failedLoginAttempt.lockout')
                .catch(function (err) {
                  log.error({ op: 'request.failedLoginAttempt.sendViolation.lockout', ip: ip, err: err })
              })
            }
          },
          function (err) {
