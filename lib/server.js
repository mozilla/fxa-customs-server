...
module.exports = function createServer(config, log) {
  ...
  if (config.ipBlocklist.enable) {
    var IPBlocklist = require('./ip_blocklist')(log)
    var ipBlocklist = new IPBlocklist()
    ipBlocklist.load(config.ipBlocklist.listPath)
      .then(function () {
        log.info({op: 'ipBlocklist.loaded', path: config.ipBlocklist.listPath})
      })
      .catch(function (err) {
        log.error({op: 'ipBlocklist.error', err: err})
      })
  }
  ...
  api.post(
    ...
    function (req, res, next) {
      ...
      // Check blocklist before retrieving records, save a lookup
      if (config.ipBlocklist.enable && ipBlocklist.contains(ip)) {
        log.debug({op: 'ipBlocklist.hit', ip: ip, action: action, list: config.listPath})
