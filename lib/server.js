...
module.exports = function createServer(config, log) {
  ...
  // Update all blocks, return the maximum block.
  function updateBlocks(action, wantsUnblock, email, emailRecord, ipEmailRecord, ipRecord, smsRecord) {
    return [
      emailRecord.update(action, !!wantsUnblock),
      (() => {
      ...
      ipRecord.update(action, email),
      (() => {
        if (smsRecord) {
          return smsRecord.update(action)
        }
        return 0
      })()
    ].reduce(max)
      ...
      fetchRecords(ip, email, phoneNumber)
        .spread(
          function (ipRecord, reputation, emailRecord, ipEmailRecord, smsRecord) {
            ...
            var retryAfter = updateBlocks(action, wantsUnblock, email, emailRecord, ipEmailRecord, ipRecord, smsRecord)
