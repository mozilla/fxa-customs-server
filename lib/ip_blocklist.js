  ...
  IPBlocklist.prototype.load = function (filePath) {
    ...
    return statFile(filePath)
      ...
      .then(function (rows) {
        ...
        var batchSize = config.ipBlocklist.batchSize
        ...
        var promiseWhile = Promise.method(function () {
          if (processedItems >= rowCount) {
            return
          }

          var loopCount = processedItems + batchSize
          if ((processedItems + batchSize) > rowCount) {
            loopCount = rowCount
          }

          for (var i = processedItems; i < loopCount; i++) {
            var parsedData = parseRow(rows[i][0])
            ...
            if (parsedData) {
              ...
              newIpsByPrefixLength[parsedData.prefixLength][parsedData.baseIp] = true
            }

            processedItems++
          }

          return Promise.delay(config.ipBlocklist.batchDelayMs)
            .then(function () {
              return promiseWhile()
            })
        })
