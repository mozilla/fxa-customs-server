...
module.exports = function (log, config) {
  ...
  IPBlocklist.prototype.contains = function (ipAddress) {
    ...
    try {
      return self.prefixLengths.some(function (prefixLength) {
        ...
        if (result) {
          endTime = Date.now()
          log.info({op: 'fxa.customs.blocklist.hit', ip: ipAddress, fileName: self.fileName, fileSize: self.fileSize,
      ...
      log.error({op: 'fxa.customs.blocklist.error', fileName: self.fileName, fileSize: self.fileSize, err: err})
      return true
    }
  }
  ...
  IPBlocklist.prototype.refresh = function () {
    ...
    log.trace({op: 'fxa.customs.blocklist.refreshList', fileName: self.fileName, fileSize: self.fileSize})
